// Lightweight QR encoder adapted from "qrcode-generator" (MIT, Kazuhiko Arase)
// Exposes createQrDataUrl(text, size)
...
function QRPolynomial(num, shift) {
  if (num.length === undefined) throw new Error("Invalid QRPolynomial data");
  let offset = 0;
...
QRPolynomial.prototype.get = function get(i) {
  return this.num[i];
};

QRPolynomial.prototype.getLength = function getLength() {
  return this.num.length;
};

QRPolynomial.prototype.multiply = function multiply(e) {
...
QRPolynomial.prototype.mod = function mod(e) {
...
QRBitBuffer.prototype.put = function put(num, length) {
  for (let i = 0; i < length; i++) {
    this.putBit(((num >>> (length - i - 1)) & 1) === 1);
  }
};

QRBitBuffer.prototype.getLengthInBits = function getLengthInBits() {
  return this.length;
};

QRBitBuffer.prototype.putBit = function putBit(bit) {
...
QRCode.prototype.makeImpl = function makeImpl(test, maskPattern) {
...
QRCode.prototype.setupPositionProbePattern = function setupPositionProbePattern(row, col) {
  for (let r = -1; r <= 7; r++) {
...
QRCode.prototype.getBestMaskPattern = function getBestMaskPattern() {
  let minLostPoint = 0;
  let pattern = 0;
  for (let i = 0; i < 8; i++) {
...
QRCode.prototype.createMovieClip = function createMovieClip(_target_mc, _instance_name, _depth) {
  // unused in this adaptation
};

QRCode.prototype.setupTimingPattern = function setupTimingPattern() {
...
QRCode.prototype.setupPositionAdjustPattern = function setupPositionAdjustPattern() {
...
QRCode.prototype.setupTypeNumber = function setupTypeNumber(test) {
...
QRCode.prototype.setupTypeInfo = function setupTypeInfo(test, maskPattern) {
...
QRCode.prototype.mapData = function mapData(data, maskPattern) {
  let inc = -1;
...
export function createQrDataUrl(text, { size = 220, margin = 2, errorCorrection = "M" } = {}) {
  ...
  const cells = moduleCount + margin * 2;
  const cellSize = Math.max(1, Math.floor(size / cells));
  const qrSize = cells * cellSize;
